...
public class KeyPair {
   ...
   // private final Signature.Type mKeyType;
    ...
    public KeyPair(Pkcs11 pkcs11, long session, byte[] ckaId) throws Pkcs11CallerException {
        ...
        // уберём заголовок ключа (см ASN.1 Basic Encoding Rules)
        CK_ATTRIBUTE[] secKeyTemplate = (CK_ATTRIBUTE[]) (new CK_ATTRIBUTE()).toArray(2);
        final NativeLongByReference keySecClass =
                new NativeLongByReference(new NativeLong(Pkcs11Constants.CKO_PRIVATE_KEY));

        secKeyTemplate[0].type = new NativeLong(Pkcs11Constants.CKA_CLASS);
        secKeyTemplate[0].pValue = keySecClass.getPointer();
        secKeyTemplate[0].ulValueLen = new NativeLong(NativeLong.SIZE);
        ByteBuffer valueSecBuffer = ByteBuffer.allocateDirect(ckaId.length);
        valueSecBuffer.put(ckaId);
        secKeyTemplate[1].type = new NativeLong(Pkcs11Constants.CKA_ID);
        secKeyTemplate[1].pValue = Native.getDirectBufferPointer(valueSecBuffer);
        secKeyTemplate[1].ulValueLen = new NativeLong(ckaId.length);
        long secKeyHandle;
        try {
            Log.v("KeyPair", "secKeyHandle");
            secKeyHandle = findObject(pkcs11, session, secKeyTemplate);
        } catch (ObjectNotFoundException e) {
            Log.v("KeyPair", "secKeyHandle KeyNotFoundException");
            throw new KeyNotFoundException();
        }
    ...
    public long getPrivateKeyHandle(Pkcs11 pkcs11, long session) throws Pkcs11CallerException {
        Log.v("KeyPairtPrivateKeyHandl", "mId" + new String(mId));
