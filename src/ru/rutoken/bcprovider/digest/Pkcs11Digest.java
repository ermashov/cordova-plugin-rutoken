...
abstract class Pkcs11Digest implements Digest {
    ...
    Pkcs11Digest(Pkcs11 pkcs11, long sessionHandle, long mechanismType) {
        mPkcs11 = Objects.requireNonNull(pkcs11);
        mSessionHandle = new NativeLong(sessionHandle);
        mMechanism = makeMechanismRSA(mechanismType);
    }

    private static CK_MECHANISM makeMechanismRSA(long type) {
        return new CK_MECHANISM(new NativeLong(type), null, new NativeLong(0));
    }
    ...
    private void update(byte[] chunk) {
        ...
        if (!mIsOperationInitialized) {
            ...
            try {
                ...
                Log.v("TAG C_DigestInit excep", e.getMessage());
            }
